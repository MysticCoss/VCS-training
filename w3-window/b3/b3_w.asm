...
;.\GoLink /files /console /debug coff b3_w.obj kernel32.dll
...
Start:
	...
	;offset -48  : char* out[alen+blen]
	;offset -40  : char* b[52]  //Include \r\n when read from console
	;offset -32  : char* a[52] //Include \r\n when read from console
	...
	mov 	rdx, info0							;"Calculator program. Choose your option wisely : ", 0
	mov 	r8d, info0len						
	mov		r9, [rbp-24]						;writtenlen. Probably discarded
	push 	0
	call 	WriteConsoleA						
	add		rsp, 40								;Shadow store + clean up paramenter	
	
	;Read option from user input
	;ReadConsoleA(hStdin, rbp-64, 3, &alen,0)
	sub 	rsp, 32								;Shadow store
	...
	lea 	r9, [rbp-8]							;&alen
	push 	0									;0
	call 	ReadConsoleA						;ReadConsoleA(hStdin, a, 52, &alen,0);
	add		rsp, 40								;Shadow store + clean up paramenter	
	...
	sub 	rsp, 32								;Shadow store
	mov		rcx, [hStdout]						;hStdout
	mov 	rdx, info1							;"Please enter first number : "...etc
	mov 	r8d, info1len						
	mov		r9, [rbp-24]						;writtenlen. Probably discarded
	push 	0
	call 	WriteConsoleA						
	...
	;a = HeapAlloc(hHeap,8,52)
	sub 	rsp, 32								;Shadow store
	mov 	rcx, [hHeap]						;hHeap
	mov 	edx, 8								;HEAP_ZERO_MEMORY 0x00000008
	mov 	r8d, 52								;number of byte allocated
	call	HeapAlloc							;a = HeapAlloc(hHeap,8,52)
	mov 	[rbp-32], rax	
	add 	rsp, 32								;Shadow store	
	
	;ReadConsoleA(hStdin, a, 52, &alen,0)
	sub 	rsp, 32								;Shadow store
	mov		rcx, [hStdin]						;hStdin
	mov 	rdx, [rbp-32]						;a
	mov 	r8d, dword 52						;52
	...
	;===========================
	;b = HeapAlloc(hHeap,8,52)
	sub 	rsp, 32								;Shadow store
	mov 	rcx, [hHeap]						;hHeap
	mov 	edx, 8								;HEAP_ZERO_MEMORY 0x00000008
	mov 	r8d, 52								;number of byte allocated
	call	HeapAlloc							;b = HeapAlloc(hHeap,8,52)
	mov 	[rbp-40], rax		
	add 	rsp, 32								;Shadow store
	...
	sub 	rsp, 32								;Shadow store
	mov		rcx, [hStdout]						;hStdout
	mov 	rdx, info2							;"Please enter second number : "
	mov 	r8d, info2len						
	mov		r9, [rbp-24]						;writtenlen. Probably discarded
	push 	0
	call 	WriteConsoleA						
	...
	;ReadConsoleA(hStdin, b,52, &blen, 0);
	sub 	rsp, 32								;Shadow store
	mov		rcx, [hStdin]						;hStdin
	mov 	rdx, [rbp-40]						;b
	mov 	r8d, dword 52						;52
	lea 	r9, [rbp-16]						;&blen
	push 	0									;0
	call 	ReadConsoleA						;ReadConsoleA(hStdin, c, 52, &blen, 0);
	add		rsp, 40								;Shadow store + clean up paramenter											
	...
	;out = HeapAlloc(hHeap,8,alen+blen)
	sub 	rsp, 32								;Shadow store
	mov 	rcx, [hHeap]						;hHeap
	mov 	edx, 8								;HEAP_ZERO_MEMORY 0x00000008
	mov 	r8d, dword [rbp-16]					;number of byte allocated
	add 	r8d, dword [rbp-8]
...
L_Print:
	...
	sub 	rsp, 32								;Shadow store
	mov		rcx, [hStdout]						;hStdout
	mov 	rdx, info3							;"Your result : "
	mov 	r8d, info3len						;length
	mov		r9, [rbp-24]		
	push 	0
