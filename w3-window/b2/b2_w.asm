...
;.\GoLink /files /console /debug coff b2_w.obj kernel32.dll
...
segment .data
	...
	nanlen equ $-nan
	odd db "Odd number sum : ", 0
	oddlen equ $-odd
	even db "Even number sum : ", 0
	evenlen equ $-even
segment .bss
	hStdin resq 1
	hStdout resq 1
...
Start:
	...
	;slen -= 2
	mov 	rax, [rbp-48]						;cut away \n but leave \r for end of string signature			
	sub 	rax, 2								;this aid further
	mov 	[rbp-48], rax
	...
	;Loop to get calculate sum of odd and even number
	;Initialization
	xor 	r15, r15 							;r15 : odd sum  (%0 == 1)
	xor 	r14, r14							;r14 : even sum (%0 == 0)
	
	mov 	r13, [rbp-8]						;alen
	mov 	r12, [rbp-32]						;ptr a
L0:
	;Comparison
	cmp 	r13, 0								;r13 is loop counter. set to array len.
												;this is a reverse loop
	jbe 	PrintOutput							;loop condition: r13 > 0
	
	;Loop
	mov 	qword r11, [r12]					;load a number in array
	;Check odd/even
	
	;prepair for div instruction: [rdx:rax] / [rcx] 2
	xor 	rdx, rdx							;clear up 64 bit
	;low 64 bit in rax 	
	mov 	rax, r11
	
	;dividend [rdx:rax] = [0:rax]
							
	mov 	rcx, 2								;divisor = 2 
	
	div 	rcx									;[rdx:rax] / [rcx] 2
	
	cmp 	rdx, 0								;check remainder
	je  	Even								;Remainder = 0 so it's even number, jump to even
	jmp 	Odd
	
Even:
	;r11 still store target number
	add 	r14, r11
	jmp 	L1
Odd:
	;r11 still store target number
...
L1:	
	;Increment
	add 	r12,8								;increase r12 to point to the next number
	dec 	r13
	jmp 	L0
