...
int CSimpleWindow::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	ctrl_list_foundfile.InsertColumn(nCol++, _T("Id"), LVCFMT_LEFT, baseColumnWidth*1);
	ctrl_list_foundfile.InsertColumn(nCol++, _T("Name"), LVCFMT_LEFT, baseColumnWidth*2);
	ctrl_list_foundfile.InsertColumn(nCol++, _T("Command line"), LVCFMT_LEFT, baseColumnWidth*3.5);
	ctrl_list_foundfile.InsertColumn(nCol++, _T("Full path"), LVCFMT_LEFT, rect.Width() - baseColumnWidth * 6.5-1);
...
void CSimpleWindow::OnButtonClick_button_search()
{
	...
	auto hr = pSvc->ExecQuery(bstr_t("WQL"), bstr_t("SELECT * FROM Win32_process"), WBEM_FLAG_FORWARD_ONLY,
		NULL, &pEnumerator);
...
void CSimpleWindow::OnSize(UINT nType, int cx, int cy)
{
	...
	const int baseColumnWidth = rect.Width() / baseColumnNumberOfUnit;
	ctrl_list_foundfile.SetColumnWidth(nCol++, baseColumnWidth * 1);
	ctrl_list_foundfile.SetColumnWidth(nCol++, baseColumnWidth * 2);
	ctrl_list_foundfile.SetColumnWidth(nCol++, baseColumnWidth * 3);
	ctrl_list_foundfile.SetColumnWidth(nCol++, rect.Width() - baseColumnWidth * 6 - 1);
}
